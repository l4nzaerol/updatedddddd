...
class RealisticAlkansyaSeeder extends Seeder
{
    /**
     * Run the database seeds.
     * Creates 3 months of realistic Alkansya production data with material replenishment
     */
    public function run(): void
    {
        $this->log('🌱 Seeding 3 months of realistic Alkansya production data...');
        ...
        if ($bomMaterials->isEmpty()) {
            $this->log('❌ No BOM materials found for Alkansya. Please run AccurateMaterialsSeeder first.');
            return;
        }
        ...
        // Initialize stock levels for materials with higher initial stock
        $stockLevels = [];
        foreach ($bomMaterials as $bomMaterial) {
            ...
            if ($inventory) {
                // Set higher initial stock for realistic production
                $initialStock = $this->calculateInitialStock($material, $bomMaterial);
                $inventory->current_stock = $initialStock;
                $inventory->save();
                $stockLevels[$material->material_id] = $initialStock;
            } else {
        ...
        $lastReplenishmentDate = $startDate->copy();

        while ($currentDate->lte($endDate)) {
            // Skip Sundays (dayOfWeek = 0)
            if ($currentDate->dayOfWeek !== 0) {
                // Replenish materials every 2 weeks
                if ($currentDate->diffInDays($lastReplenishmentDate) >= 14) {
                    $this->replenishMaterials($bomMaterials, $stockLevels, $currentDate);
                    $lastReplenishmentDate = $currentDate->copy();
                }

                $result = $this->createDailyOutput($currentDate, $bomMaterials, $stockLevels, $alkansyaProduct);
                if ($result) {
                    $totalCreated++;
                }
            }
    ...
    /**
     * Calculate initial stock based on material type and BOM usage
     */
    private function calculateInitialStock($material, $bomMaterial)
    {
        // Base stock calculation - enough for 3 months of production
        $baseProduction = 300; // Average daily production
        $daysInThreeMonths = 90;
        $totalProduction = $baseProduction * $daysInThreeMonths;
        
        $requiredStock = $bomMaterial->quantity_per_product * $totalProduction;
        
        // Add buffer (50% extra)
        $bufferStock = $requiredStock * 1.5;
        
        // Round up to reasonable numbers
        if ($bufferStock < 100) {
            return 1000;
        } elseif ($bufferStock < 1000) {
            return 5000;
        } else {
            return round($bufferStock);
        }
    }

    /**
     * Replenish materials to maintain production
     */
    private function replenishMaterials($bomMaterials, &$stockLevels, $date)
    {
        $this->log("🔄 Replenishing materials on {$date->format('Y-m-d')}");
        
        foreach ($bomMaterials as $bomMaterial) {
            $material = $bomMaterial->material;
            $currentStock = $stockLevels[$material->material_id];
            $reorderLevel = $material->reorder_level * 10; // 10x reorder level
            
            if ($currentStock < $reorderLevel) {
                $replenishmentQuantity = $this->calculateInitialStock($material, $bomMaterial);
                $stockLevels[$material->material_id] += $replenishmentQuantity;
                
                // Update inventory
                $inventory = Inventory::where('material_id', $material->material_id)->first();
                if ($inventory) {
                    $inventory->current_stock = $stockLevels[$material->material_id];
                ...
                InventoryTransaction::create([
                    ...
                    'reference' => 'REPLENISH-' . $date->format('Ymd') . '-' . rand(1000, 9999),
                    'remarks' => "Material replenishment for continued production",
                    'timestamp' => $date->setTime(rand(8, 10), rand(0, 59)),
                    'unit_cost' => $material->standard_cost,
                    'total_cost' => $material->standard_cost * $replenishmentQuantity,
                    'user_id' => 1,
                    'metadata' => json_encode([
                        'replenishment_date' => $date->format('Y-m-d'),
     ...
     */
    private function createDailyOutput($date, $bomMaterials, &$stockLevels, $alkansyaProduct)
    {
        ...
        DB::beginTransaction();
        try {
            // Calculate materials used and update inventory
            $materialsUsed = [];
            $totalCost = 0;
            ...
            $alkansyaProduct->stock += $quantityProduced;
            $alkansyaProduct->save();

            // Create production output transaction (for tracking)
            InventoryTransaction::create([
